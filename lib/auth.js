...
export async function createSessionFor(cookieName, payload, days = 30) {
  ...
  cookies().set(cookieName, token, {
    httpOnly: true,
    // SameSite=None + Secure is required so the cookie works when the app is
    // embedded in a third-party iframe (e.g. Emergent preview dashboard).
    sameSite: 'none',
    secure: true,
    path: '/',
...
export function clearSessionFor(cookieName) {
  cookies().set(cookieName, '', { path: '/', maxAge: 0, sameSite: 'none', secure: true })
}
