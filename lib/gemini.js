...
// Model aliases — use latest stable Gemini 2.x with graceful fallback
...
export function geminiPro() {
  return genAI().getGenerativeModel({ model: PRO_MODEL })
}
...
// Try generating with primary model; if "not found"/404, retry with fallback alias
async function safeGenerate(modelName, parts) {
  // Quota-safe fallback chain: try requested model, then 2.5 family, then latest aliases.
  const fallbackChain = [
    modelName,
    'gemini-2.5-flash',
    'gemini-flash-latest',
  ...
  for (const name of fallbackChain) {
    try {
      ...
      return await m.generateContent(parts)
    } catch (e) {
      lastErr = e
      const msg = (e && e.message) || ''
      // Fall through only on model availability/quota errors, NOT validation errors (400)
      const isAvailabilityErr = /\[404|not found|not supported|is not supported for generateContent|RESOURCE_EXHAUSTED|\[429|quota/i.test(msg)
      if (!isAvailabilityErr) throw e
      console.warn(`[gemini] model ${name} unavailable (${msg.slice(0, 120)}…), trying next…`)
    }
  }
  throw lastErr
}
...
- Notes: ${visit.notes || 'N/A'}`
    const res = await safeGenerate(FLASH_MODEL, prompt)
...
export async function clinicalSummary(patientData) {
  try {
    ...
    const res = await safeGenerate(PRO_MODEL, prompt)
    return res.response.text().trim()
  } catch (e) {
...
export async function ocrPrescription(imageBase64) {
  try {
    ...
    const res = await safeGenerate(FLASH_MODEL, [
